...
public class LecturerController {
    ...
    @GetMapping("/searchLecturer-by-name/{lecturerName}")
    public ResponseEntity<?> getLecturerByName(@PathVariable String lecturerName) {
        try {
            List<LecturerEntity> lecturers = service.searchLecturerByName(lecturerName);
            if (lecturers.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No lecturers found with name: " + lecturerName);
            }
            return ResponseEntity.ok(lecturers);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred while searching for lecturers.");
